 ...
 */
public class GarageWithoutException {
	 ...
	 * @throws GarageFullException             Failed to add because the garage is
	 *                                         full.
	 * @throws VehicleAlreadyInGarageException Failed to add the vehicle because it
	 *                                         is already in the garage.
	 */
	public void addVehicle(Moveable vehicle) {
	...
	private boolean isVehiclePresent(Moveable vehicle) {
		...
		for (int i = 0; i < MAX && !found; i++) {
			if (vehicles[i] != null && vehicles[i].hashCode() == vehicle.hashCode() && vehicles[i].equals(vehicle)) {
				found = true;
			}
		}
		return found;
	...
	public String toString() {
		return "Garage [vehicles=" + Arrays.toString(vehicles) + ", counter=" + counter + "]";
	}
