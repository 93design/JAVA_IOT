...
public class PalinDroomApp {
	...
	public static boolean isZinPalindroom(String string) {
		...
		temp = temp.replace("'", "");
		temp = temp.replace(" ", "");
		/*String stringWithoutSpace = deleteStringInAString(string, " ");
		String stringWithoutComma = deleteStringInAString(stringWithoutSpace, ",");
		String stringWithoutQuote = deleteStringInAString(stringWithoutComma, "'");
		*/
...
/*
	public static String deleteStringInAString(String str, String c) {

		StringBuilder stringBuilderOnlyLetters = new StringBuilder(str);
		for (int i = 0; i < stringBuilderOnlyLetters.length(); i++) {
			int pos = stringBuilderOnlyLetters.indexOf(c);
			if (pos != -1) {
				stringBuilderOnlyLetters.deleteCharAt(pos);
				stringBuilderOnlyLetters.append(' ');
			}
		}
		return stringBuilderOnlyLetters.toString().trim();
	}
	*/

	public static boolean isUrl(String string) {
		//3)	Ga na of er een url is ingegeven of niet. (Een url start met http://). 
